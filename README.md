...
Several cards' Attributions sections note they were adapted from the same author's other private tooling and research (decision-review practice, an AI-PM training curriculum, a market-sizing prompt loop), from the same author's other public work (essays from Dean Peters's Substack, *Confessions of a Feature Factory Escapee*), or from public frameworks built by others (the MITRE Innovation Toolkit's Premortem technique). Same-author adaptations carry no license conflict — a license grants permissions to *other* people, and a copyright holder is free to adapt and relicense their own work. Those Attributions lines are lineage — a breadcrumb back to where an idea started — not a license dependency.
